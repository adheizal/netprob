...
5. SQLite is the API/UI source of truth. When configured, VictoriaMetrics receives enriched ping samples, bounded MTR summaries, failed-ping status events, latest-route MTR hop metadata and numeric per-hop measurements, and 30-second controller inventory snapshots. Probe labels retain stable IDs and add hostname, address, link, region, and provider metadata. Hop metadata may include the hop number, hostname, and IP; never export probe error text or numeric measurements as labels.
